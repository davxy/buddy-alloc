...
static struct list_link *buddy_get(
    struct buddy_sys *ctx, 
    struct list_link *block,
    size_t order
) {
...
void *buddy_alloc(struct buddy_sys *ctx, unsigned int order)
{
    ...
    for (i = order; i <= ctx->order_max; i++) {
        if (ctx->free_area[i].list.next != &ctx->free_area[i].list) {
        ...
        size_t unitsize, int busy)
{
    ...
    for (i = 0; i < ctx->order_max; i++) {
        // divide by the number of blocks of order i by 2^(i+1) 
        int count = (nunits >> (i+1));

        // number of required unsigned longs to hold the map
